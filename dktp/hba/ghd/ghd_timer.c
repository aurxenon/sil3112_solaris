...
static struct modlmisc modlmisc = {
	...
	"Solaris 11 CCB Timeout Utility Routines"
};
...
_init()
{
	...
	sol11ghd_timer_init(&tmr_conf, 0);
	return ((err = mod_install(&modlinkage)) != 0)
		sol11ghd_timer_fini(&tmr_conf);
	return (err);
}
...
void
sol11ghd_timer_newstate(ccc_t *cccp, gcmd_t *gcmdp, gtgt_t *gtgtp,
    gact_t action, int calltype)
{
	...
	for (;;) {
		...
		default:
			...
			GDBG_ERROR(("?sol11ghd_timer_newstate: HBA reset failed "
...
sol11ghd_timeout_softintr(caddr_t arg)
{
	...
	if (cccp->ccc_timeout_pending) {
		...
		sol11ghd_timer_poll(cccp, GHD_TIMER_POLL_ALL);
...
void
sol11ghd_timer_poll(ccc_t *cccp, gtimer_poll_t calltype)
{
	gcmd_t	*gcmdp;
	gact_t	 action;
	...
	while (gcmdp = sol11ghd_timeout_get(cccp)) {
		...
		default:
			...
			GDBG_ERROR(("sol11ghd_timer_poll: invalid state %d\n",
			    gcmdp->cmd_state));
			return;
		...
		sol11ghd_timer_newstate(cccp, gcmdp, gcmdp->cmd_gtgtp, action,
		    GHD_TIMEOUT);
...
void
sol11ghd_timer_init(tmr_t *tmrp, long ticks)
{
	int	indx;
...
void
sol11ghd_timer_fini(tmr_t *tmrp)
{
...
int
sol11ghd_timer_attach(ccc_t *cccp, tmr_t *tmrp,
...
void
sol11ghd_timer_detach(ccc_t *cccp)
{
	...
	GDBG_ERROR(("sol11ghd_timer_detach: corrupt list, cccp=0x%p\n",
	    (void *)cccp));
...
/*
 *
 * sol11ghd_timer_start()
 *
 *	Add a CCB to the CCB timer list.
 */

void
sol11ghd_timer_start(ccc_t *cccp, gcmd_t *gcmdp, long cmd_timeout)
{
	ulong_t	lbolt;
...
void
sol11ghd_timer_stop(ccc_t *cccp, gcmd_t *gcmdp)
{
