...
/*ARGSUSED*/
gtgt_t *
sol11ghd_target_init(dev_info_t	*hba_dip,
		dev_info_t	*tgt_dip,
		ccc_t		*cccp,
		...
		uchar_t		 lun)
{
	...
	while (gdevp != NULL) {
		if (gdevp->gd_target == target && gdevp->gd_lun == lun) {
			GDBG_WAITQ(("sol11ghd_target_init(%d,%d) found gdevp 0x%p"
				" gtgtp 0x%p max %lu\n",
	...
	GDBG_WAITQ(("sol11ghd_target_init(%d,%d) new gdevp 0x%p gtgtp 0x%p"
		    " max %lu\n", target, lun, gdevp, gtgtp, maxactive));
...
/*ARGSUSED*/
void
sol11ghd_target_free(dev_info_t	*hba_dip,
		dev_info_t	*tgt_dip,
		ccc_t		*cccp,
		gtgt_t		*gtgtp)
{
	...
	GDBG_WAITQ(("sol11ghd_target_free(%d,%d) gdevp-0x%p gtgtp 0x%p\n",
	...
	if (gdevp->gd_ninstances == 1) {
		GDBG_WAITQ(("sol11ghd_target_free: N=1 gdevp 0x%p\n", gdevp));
		/*
		...
		GDBG_WAITQ(("sol11ghd_target_free: N=0 gdevp 0x%p\n", gdevp));
...
void
sol11ghd_waitq_shuffle_up(ccc_t *cccp, gdev_t *gdevp)
{
	gcmd_t	*gcmdp;

	ASSERT(mutex_owned(&cccp->ccc_waitq_mutex));

	GDBG_WAITQ(("sol11ghd_waitq_shuffle_up: cccp 0x%p gdevp 0x%p N %ld "
	...
	for (;;) {
		...
		if (GDEV_NACTIVE(gdevp) >= GDEV_MAXACTIVE(gdevp)) {
			GDBG_WAITQ(("sol11ghd_waitq_shuffle_up: N>MAX gdevp 0x%p\n",
				gdevp));
			return;
		}
		...
		if (gdevp->gd_ninstances > 1 && GDEV_NACTIVE(gdevp) > 0) {
			GDBG_WAITQ(("sol11ghd_waitq_shuffle_up: multi gdevp 0x%p\n",
				gdevp));
			return;
		}
...
void
sol11ghd_waitq_delete(ccc_t *cccp, gcmd_t *gcmdp)
{
	...
	GDBG_WAITQ(("sol11ghd_waitq_delete: gcmdp 0x%p qp 0x%p level %ld\n",
...
int
sol11ghd_waitq_process_and_mutex_hold(ccc_t *cccp)
{
	gcmd_t	*gcmdp;
	int	 rc = FALSE;
...
void
sol11ghd_waitq_process_and_mutex_exit(ccc_t *cccp)
{
	ASSERT(mutex_owned(&cccp->ccc_hba_mutex));
	ASSERT(mutex_owned(&cccp->ccc_waitq_mutex));

	GDBG_WAITQ(("sol11ghd_waitq_process_and_mutex_exit: cccp 0x%p\n", cccp));
