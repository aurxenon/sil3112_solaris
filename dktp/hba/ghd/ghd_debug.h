...
/*PRINTFLIKE1*/
extern void sol11ghd_err(const char *fmt, ...) __PRINTFLIKE(1);
...
#else
#define	GDBG_PRF(fmt)	sol11ghd_err fmt
#endif

#if defined(GHD_DEBUG) || defined(__lint)

#define	GDBG_FLAG_CHK(flag, fmt) if (sol11ghd_debug_flags & (flag)) GDBG_PRF(fmt)
...
#else
#define	GDBG_ERROR(fmt)	sol11ghd_err fmt
#endif
