...
void
sol11ghd_complete(ccc_t *cccp, gcmd_t *gcmdp)
{
			...
			void (*)(caddr_t), caddr_t))
{
	...
	if (sol11ghd_timer_attach(cccp, tmrp, timeout_func) == FALSE) {
...
void
sol11ghd_unregister(ccc_t *cccp)
{
	sol11ghd_timer_detach(cccp);
	ddi_remove_intr(cccp->ccc_hba_dip, 0, cccp->ccc_iblock);
...
int
sol11ghd_intr(ccc_t *cccp, void *intr_status)
{
	...
	GDBG_INTR(("sol11ghd_intr(): cccp=0x%p status=0x%p\n",
		cccp, intr_status));
	...
	for (;;) {
		...
		if (sol11ghd_waitq_process_and_mutex_hold(cccp)) {
		...
		GDBG_INTR(("sol11ghd_intr(): done cccp=0x%p status=0x%p rc %d\n",
			cccp, intr_status, rc));
		/*
		 * Release the mutexes in the opposite order that they
		 * were acquired to prevent requests queued by
		 * sol11ghd_transport() from getting hung up in the wait queue.
		 */
	...
	void	*intr_status)
{
	...
	while (!got_it) {
		...
		(void) sol11ghd_waitq_process_and_mutex_hold(cccp);
		mutex_exit(&cccp->ccc_waitq_mutex);

		/*
		 * Process the first of any timed-out requests.
		 */
		sol11ghd_timer_poll(cccp, GHD_TIMER_POLL_ONE);
...
int
sol11ghd_tran_abort(ccc_t *cccp, gcmd_t *gcmdp, gtgt_t *gtgtp, void *intr_status)
{
	gact_t	 action;
	int	 rc;
...
int
sol11ghd_tran_abort_lun(ccc_t *cccp,	gtgt_t *gtgtp, void *intr_status)
{
	int	 rc;
...
int
sol11ghd_tran_reset_target(ccc_t *cccp, gtgt_t *gtgtp, void *intr_status)
{
	int rc = TRUE;
...
int
sol11ghd_tran_reset_bus(ccc_t *cccp, gtgt_t *gtgtp, void *intr_status)
{
	int	rc;
		...
		void	*intr_status)
{
	...
	if (polled) {
		...
		GDBG_START(("sol11ghd_transport: polled"
		...
		GDBG_START(("sol11ghd_transport: non-polled"
			" cccp 0x%p gdevp 0x%p gtgtp 0x%p gcmdp 0x%p\n",
				cccp, gdevp, gtgtp, gcmdp));
	}
#endif
	...
	sol11ghd_timer_start(cccp, gcmdp, timeout);
	 ...
	 */
	sol11ghd_waitq_shuffle_up(cccp, gdevp);
	...
	if (!polled) {
		...
		if (!mutex_tryenter(&cccp->ccc_hba_mutex)) {
			...
			GDBG_START(("sol11ghd_transport: !mutex cccp 0x%p\n", cccp));
			mutex_exit(&cccp->ccc_waitq_mutex);
			return (TRAN_ACCEPT);
		}
		GDBG_START(("sol11ghd_transport: got mutex cccp 0x%p\n", cccp));
	...
	GDBG_START(("sol11ghd_transport: polled done cccp 0x%p\n", cccp));
	return (TRAN_ACCEPT);
}
...
/*
 * freeze the HBA waitq output (see sol11ghd_waitq_process_and_mutex_hold),
...
ghd_trigger_reset_notify(ccc_t *cccp)
{
	...
	gcmdp = sol11ghd_gcmd_alloc((gtgt_t *)NULL, 0, TRUE);
