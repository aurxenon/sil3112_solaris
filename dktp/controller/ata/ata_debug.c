...
void
dump_sol11ata_drv(ata_drv_t *P)
{


	sol11ghd_err("ctlp 0x%p &ata_id 0x%p flags 0x%x pciide dma 0x%x\n",
	...
	sol11ghd_err("ata &scsi_dev 0x%p &scsi_inquiry 0x%p &ctl_obj 0x%p\n",
		&P->ad_device, &P->ad_inquiry, &P->ad_ctl_obj);

	sol11ghd_err("ata rd cmd 0x%x wr cmd 0x%x acyl 0x%x\n",
		P->ad_rd_cmd, P->ad_wr_cmd, P->ad_acyl);

	sol11ghd_err("ata bios cyl %d hd %d sec %d  phs hd %d sec %d\n",
...
dump_sol11ata_pkt(ata_pkt_t *P)
{
	...
	sol11ghd_err("ata cdb 0x%x scb 0x%x bpb 0x%x wrt cnt 0x%x\n",
		P->ap_cdb, P->ap_scb, P->ap_bytes_per_block, P->ap_wrt_count);
	sol11ghd_err("atapi cdbp 0x%p cdb len %d cdb pad %d\n",
