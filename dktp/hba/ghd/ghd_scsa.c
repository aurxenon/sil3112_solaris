...
/*ARGSUSED*/
void
sol11ghd_tran_sync_pkt(struct scsi_address *ap, struct scsi_pkt *pktp)
{
	...
	if (gcmdp->cmd_dma_handle) {
		...
		if (status != DDI_SUCCESS) {
			cmn_err(CE_WARN, "sol11ghd_tran_sync_pkt() fail\n");
		}
	}
}
...
/*ARGSUSED*/
void
sol11ghd_pktfree(ccc_t		*cccp,
	struct scsi_address	*ap,
	struct scsi_pkt		*pktp)
{
	GDBG_PKT(("sol11ghd_pktfree: cccp 0x%p ap 0x%p pktp 0x%p\n",
	    cccp, ap, pktp));
