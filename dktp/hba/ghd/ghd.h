...
/*
 * types of sol11ghd_timer_poll() invocations
 */
...
void	sol11ghd_unregister(ccc_t *cccp);

int	sol11ghd_transport(ccc_t *cccp, gcmd_t *cmdp, gtgt_t *gtgtp,
	    ulong_t timeout, int polled, void *intr_status);

int	sol11ghd_tran_abort(ccc_t *cccp, gcmd_t *cmdp, gtgt_t *gtgtp,
	    void *intr_status);
 ...
 */
gcmd_t	*sol11ghd_gcmd_alloc(gtgt_t *gtgtp, int ccblen, int sleep);

/*
 * Free the gcmd_t wrapper and HBA private area
 */
void	sol11ghd_gcmd_free(gcmd_t *gcmdp);
...
int	sol11ghd_timer_attach(ccc_t *cccp, tmr_t *tmrp,
	    int (*timeout_func)(void *handle, gcmd_t *, gtgt_t *,
	    gact_t, int));
void	sol11ghd_timer_detach(ccc_t *cccp);
void	sol11ghd_timer_fini(tmr_t *tmrp);
void	sol11ghd_timer_init(tmr_t *tmrp, long ticks);
void	sol11ghd_timer_newstate(ccc_t *cccp, gcmd_t *cmdp, gtgt_t *gtgtp,
	    gact_t action, int calltype);
void	sol11ghd_timer_poll(ccc_t *cccp, gtimer_poll_t calltype);
void	sol11ghd_timer_start(ccc_t *cccp, gcmd_t *cmdp, long cmd_timeout);
void	sol11ghd_timer_stop(ccc_t *cccp, gcmd_t *cmdp);
...
gtgt_t	*sol11ghd_target_init(dev_info_t *, dev_info_t *, ccc_t *, size_t,
	    void *, ushort_t, uchar_t);
void	 sol11ghd_target_free(dev_info_t *, dev_info_t *, ccc_t *, gtgt_t *);
...
/*
 * The values for the calltype arg for the sol11ghd_timer_newstate() function,
...
{						\
	sol11ghd_waitq_delete(cccp, gcmdp);		\
